...
const regulationCategories = [
  "Шлем", "Форма", "Жилет", "Рюкзак", "Осн. оружие", "Вторичное оружие", "Пистолет", "Гранаты", "ПНВ", "Доп",
  "Жилеты", "Рюкзаки", "Головные уборы", "Защита лица", "Вооружение", "Процее", "Дополнительно", "Прочее",
];
...
function RegulationEditor({ regulation, onChange, onRemove, removable, isMedicine = false }: { regulation: ManualRegulation; onChange: (next: ManualRegulation) => void; onRemove?: () => void; removable?: boolean; isMedicine?: boolean }) {
  ...
  const pasteTable = (event: ClipboardEvent<HTMLElement>) => {
    ...
    const categoryKey = (value: string) => value.toLowerCase().replace(/ё/g, "е").replace(/[\s.:]/g, "");
